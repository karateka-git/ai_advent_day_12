...
Ниже тот же взгляд “по шагам”, но уже с упором именно на изменения `day_11`: layered memory, pending-flow, ручное управление и utility allocator.
...
Это важно, потому что после `day_11` памятью теперь можно управлять прямо из CLI, а не только через автоматическое извлечение.
...
Это одно из главных архитектурных изменений `day_11`.
